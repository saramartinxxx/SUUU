import { b64_encode, define, navigatePages, toFixed } from '../utils.js';
...
define('app-poster', {
    ...
    mount() {
        ...
        <div class="flex-none w-28 lg:w-32 fade_in slide_right">
            <div class="relative aspect-[2/3.2] rounded-lg overflow-hidden group border-zinc-500/60 border"><img alt="${this.__data?.name ?? this.__data?.title}"
                    class="w-full h-full object-cover hover:cursor-pointer hover:scale-105 transition border-0"
                    src="https://images.tmdb.org/t/p/w400${this.__data?.poster_path}">
                <div
            ...
            ${this.dataset.notitle ? '' : `
                <div class="flex w-full items-center justify-center mt-1">
                    <span class="text-[12px] md:text-sm lg:text-base text-center truncate px-2 opacity-80">${this.__data?.name ?? this.__data?.title}</span>
                </div>
                `}
        </div>`;
